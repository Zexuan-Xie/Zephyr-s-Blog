...
export class BlogBackendClient {
  ...
  async exportBackup(args: { outputDir: string }): Promise<{ file_path: string; node_count?: number; file_count: number }> {
    ...
    const filePath = path.join(args.outputDir, `aeolian-backup-${timestamp}.json`);
