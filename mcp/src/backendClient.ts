...
export class BlogBackendClient {
  ...
  async exportBackup(args: { backupDir: string; label?: string }): Promise<{ file_path: string; node_count?: number; file_count: number }> {
    const outputDir = await resolveBackupOutputDir(args.backupDir, args.label);
