...
- **Author Workspace Content Tree ordering** uses desktop drag-and-drop only within the current parent. Stage 2 mobile is no-regression sanity only; Stage 3 or later may expose explicit mobile `Move up` / `Move down` actions. Neither interaction reparents a node.
...
- **File editor status** remains fixed in the editor header. Stage 2 shows only `草稿` / `已发布` with manual-save feedback; Stage 3 autosave state and version access must not use recurring Toasts while the Author writes.
...
- **Collapsed Directory attention** in Stage 2 should stay minimal and only distinguish ordinary Draft/Published descendants where useful. Stage 3 may add higher-priority Save failed / unpublished Changes attention; it never displays noisy counts.
