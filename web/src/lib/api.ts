...
const commentSchema: z.ZodType<CommentItem> = z.lazy(() =>
  z.object({
    id: z.string(),
...
const entrySchema = z
  .union([legacyContentEntrySchema, directoryEntrySchema, fileEntrySchema])
  .transform(toContentEntry);
...
const openApiDirectorySchema = z.object({
  node: nodeSchema
    .extend({ kind: z.literal("directory") })
    .nullable()
    .optional(),
...
const directorySchema: z.ZodType<DirectoryPayload> = z
  .union([legacyDirectorySchema, openApiDirectorySchema])
  .transform((directory) => {
    ...
    const path = normalizeAbsolutePath(
      directory.path ?? directory.node?.path ?? "/",
    );
...
const moveRedirectSchema = z.object({
  old_path: z.string(),
  new_path: z.string(),
});

const movePreviewSchema: z.ZodType<MovePreviewResponse> = z.object({
  node_id: z.string(),
  destination_path: z.string(),
  affected_paths: z.array(z.string()).default([]),
  redirects: z.array(moveRedirectSchema).default([]),
  blocked_reasons: z.array(z.string()).default([]),
});

const reorderChildrenResponseSchema: z.ZodType<ReorderChildrenResponse> =
  z.object({
    parent_id: z.string(),
...
const fileSchema: z.ZodType<FilePayload> = z
  .union([legacyFileSchema, openApiFileSchema])
  .transform((file) => {
...
const resolveSchema: z.ZodType<ResolvePayload> = z
  .union([openApiResolveSchema, fileSchema, directorySchema])
  .transform((payload) => {
...
const recentSchema = z.object({
  items: z.array(
    z
      .union([legacyContentEntrySchema, fileEntrySchema])
      .transform(toContentEntry),
  ),
});
...
const openApiSearchResultSchema = z
  .object({
...
const searchResultSchema: z.ZodType<SearchResult> = z.union([
  legacySearchResultSchema,
  openApiSearchResultSchema,
]);
...
function toContentEntry(
  entry:
    | z.infer<typeof legacyContentEntrySchema>
...
export function likeFile(fileId: string): Promise<LikeState> {
  return requestJson(
    `/files/${encodeURIComponent(fileId)}/like`,
    likeStateSchema,
    jsonAuthInit("PUT"),
  );
}
...
export function updateAdminNode(
  nodeId: string,
  input: UpdateAdminNodeInput,
): Promise<AdminNodeDetail> {
  return requestJson(
    `/admin/nodes/${encodeURIComponent(nodeId)}`,
    adminNodeDetailSchema,
    jsonAuthInit("PATCH", input),
  );
}

export function reorderAdminChildren(
  parentId: string,
  input: ReorderChildrenInput,
): Promise<ReorderChildrenResponse> {
  return requestJson(
    `/admin/nodes/${encodeURIComponent(parentId)}/children/order`,
    reorderChildrenResponseSchema,
    jsonAuthInit("PUT", input),
  );
}

export function previewAdminMove(
  nodeId: string,
  input: MoveNodeInput,
): Promise<MovePreviewResponse> {
  return requestJson(
    `/admin/nodes/${encodeURIComponent(nodeId)}/move-preview`,
    movePreviewSchema,
    jsonAuthInit("POST", input),
  );
}

export function moveAdminNode(
  nodeId: string,
  input: MoveNodeInput,
): Promise<AdminNodeDetail> {
  return requestJson(
    `/admin/nodes/${encodeURIComponent(nodeId)}/move`,
    adminNodeDetailSchema,
    jsonAuthInit("POST", input),
  );
}
...
export async function deleteAdminNode(nodeId: string): Promise<void> {
  const response = await fetch(
    `${apiBase}/admin/nodes/${encodeURIComponent(nodeId)}`,
    jsonAuthInit("DELETE"),
  );
  if (!response.ok) {
    throw new Error(
...
export function refreshEmbedding(fileId: string): Promise<EmbeddingState> {
  return requestJson(
    `/admin/files/${encodeURIComponent(fileId)}/refresh-embedding`,
...
export async function rebuildSearchIndex(): Promise<void> {
  const response = await fetch(
    `${apiBase}/admin/search-index/rebuild`,
    jsonAuthInit("POST"),
  );
  if (!response.ok) {
    throw new Error(
...
export async function deleteAsset(assetId: string): Promise<void> {
  const response = await fetch(
    `${apiBase}/admin/assets/${encodeURIComponent(assetId)}`,
    jsonAuthInit("DELETE"),
  );
  if (!response.ok) {
    throw new Error(
